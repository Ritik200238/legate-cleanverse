...
|---|-----------|-------|---------------|------------------------|
| 1 | **CVI (A-Pass)** | 🟢 DEEP | Core condition: both sender AND recipient must hold valid A-Pass (`POST /query_apass`, `POST /verify_apass`); agents must prove a verified principal. Real tier/subTier (0–99) + group/subGroup fields drive corridor access — not an invented 3-level scheme. Revocation (`POST /update_status`) → confirmed no push webhook exists → `CVIRegistryMirror` polls and freezes escrow/mandates within one poll interval, *and* independently, any settlement attempt after revocation reverts on-chain regardless of the poller (defense-in-depth, §5.1). | All 3 boxes: core condition ✓, level-dependent behavior ✓, revocation affects positions ✓ |
...
- Accepts ONLY the A-Token (CVA) via standard `transfer`/`transferFrom`. **Real Monad testnet addresses, confirmed live 2026-08-03** via `query_deposit_atoken_list({chain:"monad"})`:
  ...
  - **aUSDC / A-Token — the only asset LegateEscrow accepts:** `0xfA96De5B8F434c26FdFf953303dD66fF80af1026`, **18 decimals** — re-verified live 2026-08-08. ⚠️ **This is not the address in Cleanverse's published docs.** They redeployed the Monad A-Token; the docs snapshot still lists the old `0xaC0893…f20D`, which is a 6-decimal contract that is no longer what `query_deposit_atoken_list` returns. Both the address *and* the decimals changed. Confirmed by calling the real API and then reading `symbol()`/`decimals()` directly off Monad RPC for both contracts. Deploy-time corridor caps are now scaled by the token's live `decimals()` rather than a hardcoded literal (see §5.1's deploy script) — a hardcoded `1e6` would have set a 0.00000001 aUSDC per-transaction cap. Full finding in `DECISIONS.md`.
...
- [ ] Confirm whether the validator address (`0xaC7e...`) is Monad-specific or shared across chains.
...
- **Demonstrate Clear User Value** — named users with named pain points, not abstract personas (§2): remittance senders paying 5–7% and waiting days; enterprises that currently have no way to let an agent spend money compliantly at all.
